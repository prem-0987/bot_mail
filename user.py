...
# -----------------------Connection Established------------------------------------------------------------
    ...
    def user_read_mail(self):
        db=self.connection()
        mycursor=db.cursor()
        
        sq="select mail_id,name,sender,receiver,subject,message,created_at from mail where receiver=%s and read_mail=1"
        record=[session['email']]
        mycursor.execute(sq,record)
        row=mycursor.fetchall()
        mycursor.close()
    ...
    def user_read_mail_update(self):
        db=self.connection()
        mycursor=db.cursor()
        sq="update mail set read_mail=0 where receiver=%s and read_mail=1"
        record=[session['email']]
        mycursor.execute(sq,record)
        db.commit()
        mycursor.close()
