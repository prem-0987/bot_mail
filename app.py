...
def user_create_email():
    ...
    flag = True
    while flag:
        texttospeech("create your Email ", file + i)
        i = i + str(1)
...
@app.route('/user_read_mail')
def user_read_mail():
    global i
    if 'email' in session:
        ob = user_operation()
        record = ob.user_read_mail()
        
        if record:
            texttospeech("your unread messages are", file + i)
            i = i + str(1)
            
            for r in record:
                texttospeech(f"sender is {r[1]}", file + i)
                i = i + str(1)
                texttospeech("subject is ", file + i)
                i = i + str(1)
                texttospeech(r[4], file + i)
                i = i + str(1)
                texttospeech("message is ", file + i)
                i = i + str(1)
                texttospeech(r[5], file + i)
                i = i + str(1)
            
            texttospeech("no more message left", file + i)
            i = i + str(1)
            
            ob.user_read_mail_update()
            return redirect(url_for('user_dashboard'))
        else:
            texttospeech("no more unread message left", file + i)
            i = i + str(1)
            return redirect(url_for('user_dashboard'))
    else:
        msg = "you are not logged in yet"
